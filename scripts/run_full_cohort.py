"""
Дополняет выборку: run_pooled_cohort.py и run_finetune.py всегда загружали
только пациентов с "исходной" схемой именования производных файлов
(фиксированные имена файлов), поэтому все 10 пациентов, использующих
альтернативную схему, встречающуюся у sub-p058...sub-p068 (T1w всей головы
вместо заранее лишённого черепа, BrMsk_CSF.nii вместо _30slices, резервный
вариант desc-CBV_cmro2 для условия задачи, BOLD_percchange только для calc),
каждый раз молча пропускались, хотя 9 из этих 10 на самом деле являются
пригодными данными.

Этот скрипт использует src.subject_loader.load_subject_robust (которая
пробует обе схемы именования) для ВСЕЙ выборки, так что "новые" пациенты
включаются наряду с исходными 25 пригодными - приближаясь к полному N=40
датасета. Только sub-p058 невосстановим (у него действительно нигде в
истории S3 нет CMRO2 control-условия в пространстве T2, проверено вручную).
Для 9 новых пациентов доступен только "calc" (производных файлов "mem" для
них вообще не существует), поэтому контраст "mem" сохраняет свой N только
по исходной выборке.

Классификация только по структуре (SimplePatchCNN, patch=9), соответствует
методологии run_pooled_cohort.py, но без ветки с условным BOLD - для новых
пациентов недоступны для скачивания сырые 4D BOLD/события ни при одной из
исследованных на данный момент схем именования, поэтому сопоставимое
сравнение по BOLD-признакам на ПОЛНОЙ расширенной выборке здесь не
проводится. Это выделяет единственный реальный вопрос, на который отвечает
данный скрипт: меняет ли вообще добавление реальных отложенных пациентов
результат классификации только по структуре.
"""
...
def main():
    ...
    for i, sub in enumerate(ALL_SUBJECTS):
        ...
        try:
            ...
            result, notes = None, ["необработанная ошибка"]
        if result is None:
    ...
    print(f"\nИспользовано пациентов: {n_used}/{len(ALL_SUBJECTS)} (>=1 контраст)")
    ...
    for contrast in CONTRASTS:
        ...
        for fold_name, (train_idx, test_idx) in {
            ...
            acc = hist["val_acc"][-1]
            print(f"  {fold_name}: n_subjects={n_subjects} точность={acc:.3f}")
    ...
    ax.set_ylim(0, 1)
    ax.set_title(f"Классификация только по структуре на полной выборке (использовано {n_used}/{len(ALL_SUBJECTS)} пациентов)")
