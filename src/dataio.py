"""
Минимальные вспомогательные функции ввода-вывода: загрузка объёмов NIfTI в
виде numpy-массивов и построение путей к производным файлам, используемых
конвейером two_modes_of_hemodynamics (именование в стиле BIDS: sub-pXXX,
task-<cond>, space-<space>, desc-<desc>).
"""
...
def simple_brain_mask(t1_volume):
    """
    Приближённая маска мозга через пороговую обработку Отсу + выделение
    наибольшей связной компоненты + заполнение дыр. Используется только для
    пациентов, для которых есть только сырой (ещё не очищенный от черепа)
    T1w и недоступен FSL/nilearn BET. Достаточно хороша, чтобы ограничить
    центры патчей тканью мозга; не замена настоящему инструменту удаления
    черепа, если важна точность удаления лица/черепа.
    """
...
def load_nifti(path):
    """Возвращает (данные как float32 ndarray, affine, объект изображения nibabel)."""
    img = nib.load(path)
    return np.asarray(img.dataobj, dtype=np.float32), img.affine, img


def sub_id(numeric_id):
    """021 -> 'sub-p021', строки с уже добавленным префиксом пропускаются без изменений."""
...
class SubjectPaths:
    """
    Строит пути к производным файлам для одного пациента, соответствуя
    соглашению об именовании из
    https://github.com/NeuroenergeticsLab/two_modes_of_hemodynamics
    (см. combined_pipeline.py, например строки 10092-10109).
